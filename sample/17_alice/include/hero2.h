...
class Hero2 : public Entity {
 ...
 public:
	Hero2(char* path, int width, int height, int x = 0, int y = 0, int h = 0, int v = 0, int vel = 2) : Entity (path, width, height, x, y, h, v, vel) {
	...
	void controlMe(){
		...
		if(!blocked){
			...
			if(key[KEY_5]) current = fire;
			else if(key[KEY_6]) current = ice;
			else if(key[KEY_7]) current = heal;
			else if(key[KEY_8]) current = thunder;
			if(key[KEY_D] && !atk && !depress_key) {
				action = 1;
				atk = true;
				timer = 1;
				depress_key = true;
			...
			if(key[KEY_F] && !depress_key_magic && !atk && inUse == NULL && energy > 0) {
		...
		if(!key[KEY_D] && depress_key) depress_key = false;
		if(!key[KEY_F] && depress_key_magic) depress_key_magic = false;
...
*/					}
	...
	void drawStatus(BITMAP *buffer, coord origin){
		for(int i = 0; i < total_blood_points; i++){
			draw_sprite(buffer, (BITMAP*)hearts[0].dat, 435 + i*20, 15);
		}
		for(int i = 0; i < blood_points; i++){
			draw_sprite(buffer, (BITMAP*)hearts[1].dat, 435 + i*20, 15);
		}

		//Stamina
		if(stamina <= total_stamina/4) rectfill(buffer, 435, 45, 435 + (int)((200.0/((float)total_stamina))*(float)stamina), 50, makecol(242, 105, 53));
		else rectfill(buffer, 435, 45, 435 + (int)((200.0/((float)total_stamina))*(float)stamina), 50, makecol(253, 188, 89));
		rect(buffer, 435, 45, 635, 50, makecol(255, 255, 255));
		
		//Energy
		rectfill(buffer, 435, 35, 435 + (int)((200.0/((float)total_energy))*(float)energy), 43, makecol(0, 0, 255));
		rect(buffer, 435, 35, 635, 43, makecol(255, 255, 255));

		
		textprintf_ex(buffer, font, 395, 5, makecol(255, 255, 255), -1, "Lv: %d", magicLv[current][0]);
		rectfill(buffer, 400, 17, 430, 43, makecol(0, 0, 0));
		rect(buffer, 400, 17, 430, 43, makecol(255, 255, 255));
		draw_sprite(buffer, (BITMAP*)magics[current].dat, 405, 20);
		
		rectfill(buffer, 400, 45, 400 + ((30/total_xp)*magicLv[current][1]), 50, makecol(255, 255, 0));
		rect(buffer, 400, 45, 430, 50, makecol(255, 255, 255));
