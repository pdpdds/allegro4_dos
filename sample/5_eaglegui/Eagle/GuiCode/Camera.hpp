...
public :
   ...
   Camera(std::string name , BMP_DRAW_TYPE bmp_draw_type , BITMAP* view , Rectangle area_to_view ,
   ...
   virtual void SetWidgetColors(const WidgetColorset& colors);/// does not set redraw flag, colors are not used
   virtual void UseWidgetColorset(WidgetColorset* colorset);/// does not set redraw flag, colors are not used
   virtual void UseIndividualColorset();/// does not set redraw flag, colors are not used
   ...
   virtual void SetViewArea(Rectangle area_to_view);// view must be set first
   ...
   virtual void SetDrawType(BMP_DRAW_TYPE type);
   virtual void SetBlenderType(ALPHA_BLENDER_TYPE type);
