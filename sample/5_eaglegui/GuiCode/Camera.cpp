...
Camera::Camera(string name) :
      ...
      take_focus(true),
      drawtype(SOLID),
      blendtype(USE_ALPHA_BLENDER)
{
   
}


Camera::Camera(string name , BMP_DRAW_TYPE bmp_draw_type , BITMAP* view , Rectangle area_to_view , Rectangle position , UINT wflags) :
...
WidgetMsg Camera::Update (double tsec) {
...
WidgetMsg Camera::CheckInputs(int msx , int msy) {
   if (flags & ENABLED) {
...
void Camera::DisplayOn(BITMAP* bmp , int x , int y) {
   ASSERT(bmp);
   if (view_bmp && (flags & VISIBLE)) {
      ...
      switch (drawtype) {
         case SOLID :
            ...
            break;
         case TRANSPARENT :
            masked_blit(view_bmp , bmp , view_area.X() , view_area.Y() , r.X() + ox , r.Y() + oy , view_area.W() , view_area.H());
            break;
         case TRANSLUCENT :
            switch (blendtype) {
               case USE_ALPHA_BLENDER :
                  trans_blit(view_bmp , bmp , view_area.X() , view_area.Y() , r.X() + ox , r.Y() + oy , view_area.W() , view_area.H());
                  break;
               case USE_ALPHA_LAYERING_BLENDER :
                  trans_layer_blit(view_bmp , bmp , view_area.X() , view_area.Y() , r.X() + ox , r.Y() + oy , view_area.W() , view_area.H());
                  break;
            }
            break;
      }
...
void Camera::SetWidgetColors(const WidgetColorset& colors) {
   wc = colors;
   wcols = &wc;
}
...
void Camera::UseWidgetColorset(WidgetColorset* colorset) {
   wcols = colorset;
}



void Camera::UseIndividualColorset() {
   wcols = &wc;
}
...
void Camera::SetView(BITMAP* bmp , Rectangle area_to_view) {
   ASSERT(bmp);
   ASSERT(bmp->w > 0);
   ASSERT(bmp->h > 0);
   view_bmp = bmp;
   SetViewArea(area_to_view);
}
...
void Camera::SetViewArea(Rectangle area_to_view) {
   ASSERT(view_bmp);
   int w = area_to_view.W();
   int h = area_to_view.H();
   if (w < 1) {w = 1;}
   if (h < 1) {h = 1;}
...
void Camera::SetDrawType(BMP_DRAW_TYPE type) {
   drawtype = type;
   SetBgRedrawFlag();
}
...
void Camera::SetBlenderType(ALPHA_BLENDER_TYPE type) {
   blendtype = type;
   SetRedrawFlag();
}
